...
namespace SDKClient.Command
{
    ...
    class Message_cmd:CommandBase
    {
        ...
        public override string Name => Protocol.ProtocolBase.message;
       
        public override void ExecuteCommand(EasyClientBase session, PackageInfo packageInfo)
        {
            var package = packageInfo as MessagePackage;
        ...
        public override void SendCommand(EasyClientBase session, PackageInfo packageinfo)
        {
            ...
            CSMessagePackage p = new CSMessagePackage();
            p.from = packageinfo.from;
            p.to = packageinfo.to;
            p.id = packageinfo.id;
            p.version = packageinfo.version;
            p.data = ((MessagePackage)packageinfo).data;
        ...
        protected override void SendCompletedCallBack(bool isRanCompleted, PackageInfo package, EasyClientBase easyClientBase, int expires = 8)
        {
            if(isRanCompleted)
            {
                ...
                if (SendDic.TryAdd(package.id, (source,package)))
                {
                    ...
                    {
#if CUSTOMSERVER
                    CSMessagePackage messagePackage = (CSMessagePackage)package;
#else
